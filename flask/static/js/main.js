...
function submitForms() {
  ...
  const selectedPackage = packageForm.querySelector('input[name="produt"]:checked');
  ...
  // Create a hidden input to store the selected package
  const hiddenInput = document.createElement('input');
  hiddenInput.type = 'hidden';
  hiddenInput.name = 'produt';
  hiddenInput.value = selectedPackage.value;

  // Append the hidden input to the details form
  const detailsForm = document.getElementById('details-form');
  detailsForm.appendChild(hiddenInput);
...
document.addEventListener("DOMContentLoaded", function() {
  document.getElementById('packageForm').addEventListener('submit', function(event) {
      ...
      .then(data => {
          if (data.error) {
              alert(data.error); // Display error if no package is selected
              return;
          }

          // Change the button text to 'Selected'
          var submitButton = document.getElementById('submitButtons');
          submitButton.textContent = 'Selected';
          submitButton.disabled = true; // Disable the button to prevent multiple clicks



          // Scroll smoothly to the additional details form
          if (additionalDetails) {
              additionalDetails.scrollIntoView({ behavior: 'smooth' });
          }
      })
      .catch(error => {
